...
def send_welcome_mail(user: User, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            logger.info(f"Attempt {attempt + 1}/{max_retries}: Sending welcome email to {user.email}")
            ...
            logger.warning(f"Attempt {attempt + 1} failed for {user.email}: {type(e).__name__}: {e}")
            
            if attempt == max_retries - 1:
                logger.error(f"Failed to send welcome email to {user.email} after {max_retries} attempts")
