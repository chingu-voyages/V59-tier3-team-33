...
class RouteService:
    ...
    def _build_payload(self, agent: any, jobs_list: list[Event]):
        ...
        payload = {
            "mode": self.mode,
            "agents": [{
                "start_location": [float(agent.place.longitude), float(agent.place.latitude)],
    ...
    def _determine_agent_and_jobs(self, events: list[Event], lodging: Lodging) -> tuple[any, list[Event]]:
        agent = None
        jobs_list = []
        jobs_source = []
        if lodging:
